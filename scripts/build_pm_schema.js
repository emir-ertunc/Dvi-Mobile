...
const VERSION = '0.2.2';
const BUILD_ID = 'phase-2c-v0.2.2-20260510';
const ROOT = process.cwd();
const INVENTORY_PATH = join(ROOT, 'data', 'form-inventory', 'pm-acroform-inventory.json');
const OUTPUT_SCHEMA = join(ROOT, 'data', 'schema', 'pm-schema.json');
const OUTPUT_AUDIT = join(ROOT, 'data', 'schema', 'pm-schema-audit.json');
const SUMMARY_PATH = join(ROOT, 'docs', 'schema', 'phase-2c-summary.md');
...
function buildSchema() {
  ...
  return {
    schemaVersion: 'phase-2c-pm-v1',
    phase: PHASE,
    version: VERSION,
    buildId: BUILD_ID,
    formType: 'PM',
    sourceInventory: 'data/form-inventory/pm-acroform-inventory.json',
    totals: {
...
function auditSchema(schema) {
  ...
  if (schema.fields.length !== EXPECTED.fields) failures.push(`PM schema field sayısı hatalı: ${schema.fields.length}`);
  if (schema.totals.widgetInstanceCount !== EXPECTED.widgets) failures.push(`PM schema widget sayısı hatalı: ${schema.totals.widgetInstanceCount}`);
  ...
  for (const field of schema.fields) {
    ...
    if (field.formType !== 'PM') failures.push(`PM dışı schema field: ${field.schemaFieldId}`);
...
function summaryMarkdown(schema, audit) {
  return `# Phase 2C Özeti: PM Schema ve Validasyon

Phase 2C, PM AcroForm envanterindeki ${schema.fields.length} alanı canonical PM schema kaydına dönüştürür ve temel validasyon kurallarını bağlar.
...
| Metrik | Değer |
| --- | ---: |
| PM schema field | ${audit.metrics.fieldCount} |
...
function run({ write }) {
  ...
  console.log(`PM schema doğrulaması geçti. field=${audit.metrics.fieldCount}, widget=${audit.metrics.widgetInstanceCount}`);
}
